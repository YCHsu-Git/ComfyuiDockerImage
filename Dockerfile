...
RUN mkdir -p /wheels \
    && if [ "${INSTALL_SAGEATTENTION}" = "true" ]; then \
        ...
        && TORCH_CUDA_ARCH_LIST="${SAGEATTENTION_ARCH_LIST}" MAX_JOBS="${MAX_JOBS}" python -m pip wheel --no-cache-dir --no-build-isolation --no-deps -w /wheels /tmp/SageAttention \
